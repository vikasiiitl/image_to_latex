...
def load_image(image_path):
    ...
    img = tf.image.resize(
        img, (int(img.shape[0]*0.8), int(img.shape[1]*0.8)), tf.image.ResizeMethod.GAUSSIAN)
...
def predict(image_path: str, tokenizer_path: str, meta_path: str, checkpoint_path: str, beam_width=2, temperature=0.3):
    ...
    result, _ = beam_evaluate(image_path, decoder, encoder, max_length,
                              tokenizer, beam_width=beam_width, temperature=temperature)
